...
| G3 | Laadstations gaven "HTTP 500 — Open Charge Map gaf HTTP 403: You must specify an API key" | Geverifieerd via Vercel MCP (project + live productie-endpoint rechtstreeks getest): de variabele `OPENCHARGEMAP_API_KEY` bestaat wél in Vercel (Production + Preview, toegevoegd 2 dagen geleden — dus geen redeploy-timing-probleem), maar de **waarde begint met `sk_live_...`** — dat is Stripe's sleutelformaat, geen Open Charge Map-sleutel. Vermoedelijk per ongeluk de verkeerde sleutel geplakt. Zelfde onderliggende klasse als bekend issue A1 in `01-testronde-30juni.md` | **OPEN — actie ligt bij Remco, bewust uitgesteld.** Juiste sleutel ophalen via https://openchargemap.org/site/develop/api en de bestaande `OPENCHARGEMAP_API_KEY`-variabele in Vercel overschrijven (de huidige waarde is niet meer terug te lezen, staat op "Sensitive" — alleen overschrijven kan). Geen codewijziging nodig. |
